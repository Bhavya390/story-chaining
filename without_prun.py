...
def rec(mtx,s_ind,t_ind,c_m,url_link):
    ...
    if len(c_m) <= 2:
        ...
        print "printing relevence start article with other articles"
        print s_r
        print "printing relevence end article with other articles"
        print t_r
        print "rec fun"
        print s_r.get_shape()
        print t_r.get_shape()
        print len(c_m)
        #print url_link(c_m[0])
        """
        ...
        if a_ind != -1:
            ...
            c_m_cpy = list(c_m)
           # print s_ind,t_ind,len(c_m),mtx.get_shape()[0],a_ind
...
def pre():
   ...
   # print(dictionary)
    ...
    i = 0
    j = 0
   # MA = np.zeros((12112,85809)) #MA is matrix
    row = []
    col = []
    data = []
